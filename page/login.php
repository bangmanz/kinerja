	...
	if(isset($_POST['btn-login'])){
		...
		if($result=$mysqli->query($sql)){
			while($ob=$result->fetch_object()){
				...
				$insertt = "insert into logs values('','".$_SESSION['username']."','".date("Y-m-d H:i:s")."','".$_SERVER['REMOTE_ADDR']."')";
				$mysqli->query($insertt);
				echo "<script> location.replace('https://ckp1800.com/dev3/'); </script>";
