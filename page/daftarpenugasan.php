<?php 
	...
	if(isset($_GET['action'])){
		if($_GET['action']=='hapus'){
			...
			if($a = $mysqli->query($sql)){
				echo "<script> location.replace('?page=daftarpenugasan'); </script>";
			}
		}
	}
	...
	if(isset($_POST['tambah'])){
		$que = "insert into penugasan values('',".TAHUN.",'".$_SESSION['viewlaporanbulanan']."','".$_SESSION['viewidlaporanbulanan']."',".$_POST['seksi'].",".$_POST['tugas'].",".$_POST['target'].",".$_POST['satuan'].",".$_POST['waktu'].",".$_POST['satuanwaktu'].",'".$_SESSION['id']."')";
		//echo $que;
...
<table class="table table-unbordered table-condensed">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<div class="col-md-4">
					<select id="id" name="id" class="form-control chosen-select2" required="required">
						<option></option>						
...
<?php
	if(isset($_POST['submit']) || (isset($_SESSION['viewidlaporanbulanan']) && isset ($_SESSION['viewlaporanbulanan']))){
		if($_SESSION['level_user']>=2 && $_SESSION['level_user']<=6){
?>
...
//	<td>".$obj2->target*$obj2->angkred."</td>
		...
		$sql2="select id_penugasan,tahun,bulan,id_pegawai,kode_seksi,id_keg,target,satuan,targetwaktu,targetwaktu,satuanwaktu,penugasan_by,nama,uraian from pegawai p, (SELECT id_penugasan,tahun,bulan,id_pegawai,a.kode_seksi,a.id_keg,target,satuan,a.targetwaktu,satuanwaktu,penugasan_by,uraian FROM `penugasan` a, masterkegiatan m, pegawai p where p.id=a.id_pegawai and a.id_keg=m.id_keg and id_pegawai='".$_SESSION['viewidlaporanbulanan']."' and bulan='".$_SESSION['viewlaporanbulanan']."' and tahun=".TAHUN.") s where p.id=s.penugasan_by";
//		echo $sql2;
		$no = 1;
		if ($result2 = $mysqli->query($sql2)) { 
			while($obj2 = $result2->fetch_object()){	
				...
				//	if($_SESSION['level_user']<=5 || $_SESSION['id']==$obj2->penugasan_by){
					if($_SESSION['id']==$obj2->penugasan_by){
						if($obj2->kode_seksi==$_SESSION['bidang']){
							echo "
								<td><a href=''><div align='center'><img src='../images/edit.png'></div></a></td>
								<td><a href='?page=daftarpenugasan&action=hapus&id=".$obj2->id_penugasan."' onclick='return confirm(\"Apakah Anda yakin akan menghapus item ini?\");'><div align='center'><img src='../images/del.png'></div></a></td>";
						}else{
							echo "<td></td><td></td>";
						}
					}else {
