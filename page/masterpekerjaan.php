<?php
	if(isset($_POST['tambah'])){
		if($_SESSION['level_user']==5){
			$sql = "insert into masterkegiatan values('','".$_POST['seksi']."','','4','".$_POST['parent']."','".$_POST['satuan']."','".$_POST['waktu']."','".$_POST['angkred']."','','".$_POST['uraian']."','".$_SESSION['id']."','0','')";
			$msg=1;
		}else if($_SESSION['level_user']<=4){
			$sql = "insert into masterkegiatan values('','".$_POST['seksi']."','','4','".$_POST['parent']."','".$_POST['satuan']."','".$_POST['waktu']."','".$_POST['angkred']."','','".$_POST['uraian']."','".$_SESSION['id']."','1','".$_SESSION['id']."')";
			$msg=2;
		}else{
			$sql = "insert into masterkegiatan values('','".$_POST['seksi']."','','4','".$_POST['parent']."','".$_POST['satuan']."','".$_POST['waktu']."','".$_POST['angkred']."','','".$_POST['uraian']."','".$_SESSION['id']."','0','')";
			$msg=1;
		...
		if($a=$mysqli->query($sql)){
			echo "<script> location.replace('?page=masterpekerjaan&msg=".$msg."'); </script>";
		}
	}
...
<?php
	if(isset($_GET['msg'])){
		if($_GET['msg']=="1"){
			echo "<script> function myFunction() {alert('Master Pekerjaan Sudah Diusulkan, silahkan tunggu approval dari supervisor');};</script>";
		}else if($_GET['msg']=="2"){
			echo "<script> function myFunction() {alert('Master Pekerjaan Sudah Ditambahkan');};</script>";
		}
	}
?>
...
<div class="container">
	...
	<table	id="mytable2"	class="table table-borderless">
		<tr  data-toggle="collapse" data-target=".demo1">
			<td colspan="6">
			<div align="right">
				<button class="btn btn-primary">Tambah Usulan Master Pekerjaan</button>
			</div>
			</td>
		</tr>
		<tr>
	            				...
	            				<form class="form-horizontal" role="form" action="?page=masterpekerjaan" method="post">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											...
											<option></option>>
												<?php
													$sql = "select * from masterkegiatan where level=0 order by kode_kegiatan";
									...
									<div class="form-group">
										<label for="inputEmail3" class="col-sm-2 control-label">
											Kode & Butir Fungsional<br>(Boleh Dikosongkan)
										</label>
										<div class="col-sm-10">
												...
												<?php
													...
													if($res = $mysqli->query($sql)){
														while($obj=$res->fetch_object()){
															...
															//echo "<option value='".$obj->kode_perka."'>".$obj->kode_perka." - ".$obj->uraian_singkat."</option>";
														}
													}
												?>						
											</select>
									...
									<!--<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="number" name="angkred" step="0.001" class="form-control" value="" placeholder="Isikan Angka Kredit.."/>
										</div>
									</div>-->
			...
			<?php			
				...
				if($ressqqq=$mysqli->query($sqqq)){
					while($rqqq=$ressqqq->fetch_object()){
						...
						echo "<td>".$rqqq->satuan."</td><td>".$rqqq->waktu."</td>";
//						echo "<td align='right'>".$rqqq->angkred."</td>";
						/*
						if($_SESSION['level_user']==5){
